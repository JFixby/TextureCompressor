...
public class CompressedGdxTextureAtlas implements Disposable {
     ...
     * @throws IOException
     */
    public CompressedGdxTextureAtlas(String internalPackFile) throws IOException {
     ...
     * @throws IOException
     */
    public CompressedGdxTextureAtlas(FileHandle packFile) throws IOException {
     ...
     */
    public CompressedGdxTextureAtlas(FileHandle packFile, boolean flip) throws IOException {
	this(packFile, packFile.parent(), flip);
    }

    public CompressedGdxTextureAtlas(FileHandle packFile, FileHandle imagesDir) throws IOException {
     ...
     * @throws IOException
     */
    public CompressedGdxTextureAtlas(FileHandle packFile, FileHandle imagesDir, boolean flip) throws IOException {
     ...
     * @throws IOException
     */
    public CompressedGdxTextureAtlas(CompressedGdxTextureAtlasData data, boolean loadAlphaTextures) throws IOException {
    ...
    private void load(CompressedGdxTextureAtlasData data) throws IOException {
	ObjectMap<GdxAtlasPage, AtlasTexturePage> pageToTexture = new ObjectMap<GdxAtlasPage, AtlasTexturePage>();
	for (GdxAtlasPage page : data.pages) {
	...
	for (GdxAtlasRegionData region : data.regions) {

	    AtlasTexturePage texture = pageToTexture.get(region.page);
	    Texture baseTexture = texture.getTexture();
	    Texture alphaTexture = texture.getAlphaTexture();
	    AtlasRegion atlasRegion = newAtlasRegion(baseTexture, region);
	    atlasRegion.setAlphaTexture(alphaTexture);

	    regions.add(atlasRegion);

	    AtlasRegion atlasAlphaRegion = newAtlasRegion(alphaTexture, region);

	    alphaRegions.add(atlasAlphaRegion);
	}
    }

    private static AtlasRegion newAtlasRegion(Texture texture, GdxAtlasRegionData region) {
	int width = region.width;
	int height = region.height;
	AtlasRegion atlasRegion = new AtlasRegion(texture, region.left, region.top, region.rotate ? height : width,
    ...
    private AtlasTexturePage loadTexture(GdxAtlasPage page, CompressedGdxTextureAtlasData data) throws IOException {
	...
	if (loadAlphaTextures) {

	    String compressedAlphaChannelFileName = page.textureFile.nameWithoutExtension()
		    + ETC1Compressor.ALPHA_CHANNEL_FILE_EXTENTION;
	    compressedAlphaChannelFile = page.textureFile.parent().child(compressedAlphaChannelFileName);

	    java.io.InputStream javaInputStream = compressedAlphaChannelFile.read();
	    GZIPInputStream zipInputStream = new GZIPInputStream(javaInputStream);
	    Gdx2DPixmap gdxPixmap = new Gdx2DPixmap(zipInputStream, Gdx2DPixmap.GDX2D_FORMAT_ALPHA);
	...
	AtlasTexturePage texture = new AtlasTexturePage(base_texture, alpha_texture, compressedAlphaChannelFile);
    ...
    private GdxSprite newSprite(AtlasRegion region) {
	if (region.packedWidth == region.originalWidth && region.packedHeight == region.originalHeight) {
	    if (region.rotate) {
		GdxSprite sprite = new GdxSprite(region);
     ...
     */
    public void dispose() {
	for (AtlasTexturePage texture : textures)
	    texture.getTexture().dispose();
	textures.clear();
    }

    static final Comparator<GdxAtlasRegionData> indexComparator = new Comparator<GdxAtlasRegionData>() {
	public int compare(GdxAtlasRegionData region1, GdxAtlasRegionData region2) {
	...
	Array<GdxSprite> sprites = new Array<GdxSprite>(this.alphaRegions.size);
	GdxSprite sprite;
	for (int i = 0, n = alphaRegions.size; i < n; i++) {
	    AtlasRegion region = alphaRegions.get(i);
